...
var InternalCountDown = function InternalCountDown(props, ref) {
    ...
    var _useState = (0, _sliced_to_array._)((0, _react.useState)(0), 2), restTimeStamp = _useState[0], setRestTime = _useState[1];
    ...
    var formatRemainTime = function formatRemainTime(t, type) {
        var ts = t;
        var rest = {
            d: 0,
            h: 0,
        ...
        return type === 'custom' ? rest : parseFormat((0, _object_spread._)({}, rest));
    };
    var parseFormat = function parseFormat(time) {
        var d = time.d;
        var h = time.h, m = time.m, s = time.s, ms = time.ms;
        var formatCache = format;
        if (formatCache.includes('DD')) {
            formatCache = formatCache.replace('DD', (0, _padzero.padZero)(d));
        } else {
            h += Number(d) * 24;
        }
        if (formatCache.includes('HH')) {
            formatCache = formatCache.replace('HH', (0, _padzero.padZero)(h));
        } else {
            m += Number(h) * 60;
        }
        if (formatCache.includes('mm')) {
            formatCache = formatCache.replace('mm', (0, _padzero.padZero)(m));
        } else {
            s += Number(m) * 60;
        }
        if (formatCache.includes('ss')) {
            formatCache = formatCache.replace('ss', (0, _padzero.padZero)(s));
        } else {
            ms += Number(s) * 1000;
        }
        if (formatCache.includes('S')) {
            var msC = (0, _padzero.padZero)(ms, 3).toString();
    ...
    return /*#__PURE__*/ _react.default.createElement(_react.default.Fragment, null, children || /*#__PURE__*/ _react.default.createElement("div", (0, _object_spread_props._)((0, _object_spread._)({
...
var CountDown = /*#__PURE__*/ _react.default.forwardRef(InternalCountDown);
