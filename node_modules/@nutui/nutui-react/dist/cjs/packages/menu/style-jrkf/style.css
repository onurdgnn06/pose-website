/* #ifdef harmony */
/* #endif */
/* #ifndef harmony */
/* #endif */
.nut-menu {
  position: relative;
}
.nut-menu.scroll-fixed {
  position: fixed;
  top: var(--nutui-menu-scroll-fixed-top, 0);
  z-index: var(--nutui-menu-scroll-fixed-z-index, 1000);
  width: 100%;
}

.nut-menu-bar {
  position: relative;
  display: flex;
  line-height: var(--nutui-menu-bar-line-height, 48px);
  background-color: var(--nutui-color-surface-2);
  box-shadow: var(--nutui-menu-bar-box-shadow, 0 2px 12px rgba(89, 89, 89, 0.12));
}
.nut-menu-bar.opened {
  z-index: var(--nutui-menu-bar-opened-z-index, 1000);
}

.nut-menu-title {
  flex: 1;
  text-align: center;
  font-size: var(--nutui-menu-title-font-size, var(--nutui-font-text));
  color: var(--nutui-menu-title-color, var(--nutui-color-title));
  min-width: 0;
  display: flex;
  align-items: center;
  justify-content: center;
  max-width: 100%;
}
.nut-menu-title-text {
  white-space: nowrap;
  overflow: hidden;
  text-overflow: ellipsis;
  display: block;
  padding: var(--nutui-menu-title-padding, 0 8px);
}
.nut-menu-title-icon {
  flex-shrink: 0;
  transition: all 0.2s linear;
}
.nut-menu-title.active {
  color: var(--nutui-menu-item-active-color, var(--nutui-color-primary, #ff6666));
}
.nut-menu-title.disabled {
  color: var(--nutui-menu-item-disabled-color, var(--nutui-color-text-disabled));
}
.nut-menu-title.active .nut-menu-title-icon {
  transform: rotate(180deg);
}